...
const { isLoggedIn, hasProfile } = storeToRefs(authStore);

const { errors, profile } = storeToRefs(store);

const { editPersonProfile, getPersonProfile } = store;
...
onMounted(async () => {
  ...
  form.birtday = profile.value.birtday.split("/").reverse().join("-");
  form.gender = profile.value.gender;
  form.country = profile.value.country;
  form.region = profile.value.region;
  form.ocupation = profile.value.ocupation;
  form.introduction = profile.value.introduction;
  form.img_path = profile.value.file;

  const quillEditor = document.querySelector(".ql-editor");
  quillEditor.innerHTML = form.introduction;
});
const handleProfile = async () => {
  ...
  await editPersonProfile(formData);
  loading.value = false;
  if (hasProfile.value) {
    router.push({ name: "pagePerson" });
  }
};
        ...
        >
          ...
          <form @submit.prevent="handleProfile">
            <div class="mb-6 relative">
              ...
              />
              <span
                class="absolute right-[195px] top-3/4 rounded bg-orange-400 text-white"
                >click me</span
              >
            </div>
            <div class="mb-6 relative">
